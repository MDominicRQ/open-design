...
The image sets `CODEX_BIN`, `CLAUDE_BIN`, and `GEMINI_BIN` to `/usr/local/bin/*`
so stale shims in the persistent `/home/open-design` volume cannot hide the
image-provided binaries. It also sets `CODEX_HOME` and `CLAUDE_CONFIG_DIR` under
`/home/open-design` so CLI auth state persists in the `open_design_home` volume.
